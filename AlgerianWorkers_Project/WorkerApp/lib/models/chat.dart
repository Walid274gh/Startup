/// Classe représentant une conversation de chat professionnel dans l'application Khidmeti-worker (Travailleurs)
/// 
/// Cette classe contient toutes les informations nécessaires pour gérer
/// la communication professionnelle entre travailleurs et clients.
class Chat {
  ...
  /// Template de réponse rapide utilisé
  final String? quickReplyTemplate;
  
  /// Indicateur si c'est une conversation de support
  ...
  const Chat({
    ...
    this.isPinned = false,
    this.unreadCount = 0,
    this.chatType = ChatType.service,
  ...
  factory Chat.fromJson(Map<String, dynamic> json) {
    return Chat(
      ...
      chatType: ChatType.values.firstWhere(
        ...
        orElse: () => ChatType.service,
      ),
      quickReplyTemplate: json['quickReplyTemplate'] as String?,
      isSupportChat: json['isSupportChat'] as bool? ?? false,
    );
  }
  ...
  @override
  String toString() {
    return 'Chat(id: $id, workerId: $workerId, userId: $userId)';
  }
  ...
  /// Vérification si c'est une conversation de service
  bool get isServiceChat => chatType == ChatType.service;

  /// Vérification si c'est une conversation de support
  bool get isSupport => chatType == ChatType.support;

  /// Vérification si c'est une conversation de négociation
  bool get isNegotiationChat => chatType == ChatType.negotiation;
  ...
  /// Obtention du nombre de messages du client
  int get clientMessageCount => 
      messages.where((m) => m.senderId == userId).length;

  /// Vérification si la conversation a un template de réponse rapide
  bool get hasQuickReplyTemplate => 
      quickReplyTemplate != null && quickReplyTemplate!.isNotEmpty;
}
...
/// Énumération des types de conversation
enum ChatType {
  service,        // Conversation de service
  support,        // Conversation de support
  negotiation,    // Conversation de négociation
  followUp,       // Suivi post-service
}

/// Classe représentant un message de chat professionnel
class ChatMessage {
  ...
  /// Indicateur si c'est un message automatisé
  ...
  /// Vérification si le message est envoyé par le client
  bool get isFromClient => senderType == SenderType.user;
  ...
  /// Vérification si le message est de type devis
  bool get isQuote => type == MessageType.quote;
  ...
  /// Vérification si le message utilise un template
  bool get usesTemplate => templateUsed != null && templateUsed!.isNotEmpty;

  /// Vérification si le message est automatisé
  bool get isAutomatedMessage => isAutomated;
