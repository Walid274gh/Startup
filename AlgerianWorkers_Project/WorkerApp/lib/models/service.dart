/// Classe représentant un service disponible dans l'application Khidmeti-worker (Travailleurs)
/// 
/// Cette classe est spécifique à l'application travailleur et contient
/// les informations nécessaires pour la gestion des compétences et services.
class Service {
  ...
  /// Nombre de demandes actives pour ce service
  final int activeRequestsCount;
  
  /// Rating moyen des travailleurs pour ce service
  final double averageRating;
  
  /// Commission en pourcentage pour ce service
  ...
  factory Service.fromJson(Map<String, dynamic> json) {
    return Service(
      ...
      activeRequestsCount: json['activeRequestsCount'] as int? ?? 0,
  ...
  /// Vérification s'il y a des demandes actives
  bool get hasActiveRequests => activeRequestsCount > 0;

  /// Vérification si le service a un bon rating
  bool get hasGoodRating => averageRating >= 4.0;

  /// Obtention du niveau de demande (0-3 étoiles)
  int get demandLevel {
    if (activeRequestsCount == 0) return 0;
    if (activeRequestsCount < 3) return 1;
    if (activeRequestsCount < 8) return 2;
    return 3;
  }

  /// Calcul de la commission en DA pour un prix donné
  double calculateCommission(double price) {
    return (price * commissionRate) / 100;
  }

  /// Vérification si le service nécessite une certification
  bool get needsCertification => requiresCertification;

  /// Obtention du niveau de difficulté en texte
